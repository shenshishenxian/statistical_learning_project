...
df <- diabetes.factor %>% add_missinglabels_mar(Outcome~.,prob=0.8) 
g_self <- SelfLearning(Outcome~.,df,
                       method=NearestMeanClassifier,
                       prior=matrix(0.5,2))
predictions <- predict(g_self, df)
confusionMatrix(as.factor(predictions), as.factor(diabetes$Outcome))
...
diabetes.data <- subset(diabetes, select = c("Pregnancies", "Glucose", "BloodPressure", "SkinThickness", "Insulin",
                                                 "BMI", "DiabetesPedigreeFunction", "Age"))
diabetes.scale.factor <- data.frame(diabetes.data, as.factor(diabetes$Outcome)) 
colnames(diabetes.scale.factor) <- colnames(diabetes)
df <- diabetes.scale.factor %>% add_missinglabels_mar(Outcome~.,prob=0.8) 
g_self <- SelfLearning(Outcome~.,df,
                       method=NearestMeanClassifier,
                       prior=matrix(0.5,2))
predictions <- predict(g_self, df)
...
diabetes.discrete.factor <- rbind(diabetes.train.discrete.factor, diabetes.test.discrete.factor)
df <- diabetes.discrete.factor %>% add_missinglabels_mar(Outcome~.,prob=0.8) 
g_self <- SelfLearning(Outcome~.,df,
                       method=NearestMeanClassifier,
                       prior=matrix(0.5,2))
predictions <- predict(g_self, df)
...
predictions[predictions <= 0.5] = 0
confusionMatrix(as.factor(predictions), as.factor(diabetes.test.discrete.scale$Outcome))
## 4.5 Semisupervised Learning
diabetes.discrete.scale.factor <- rbind(diabetes.train.discrete.scale.factor, diabetes.test.discrete.scale.factor)
df <- diabetes.discrete.scale.factor %>% add_missinglabels_mar(Outcome~.,prob=0.8) 
g_self <- SelfLearning(Outcome~.,df,
                       method=NearestMeanClassifier,
                       prior=matrix(0.5,2))
predictions <- predict(g_self, df)
